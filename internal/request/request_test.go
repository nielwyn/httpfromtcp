...
func TestRequestLineParse(t *testing.T) {
	// Test: GET Request line with chunkReader
	reader := &chunkReader{
	...
	// Test: GET Request line with path chunkReader
	reader = &chunkReader{
	...
	// Test: Invalid method (starts with /)
	r, err = RequestFromReader(strings.NewReader("/pomegranate HTTP/1.1\r\nHost: localhost:42069\r\n\r\n"))
	require.Error(t, err)
	require.Nil(t, r)

	// Test: Missing request target (only 2 parts)
	r, err = RequestFromReader(strings.NewReader("GET HTTP/1.1\r\nHost: localhost:42069\r\n\r\n"))
	require.Error(t, err)
	require.Nil(t, r)
}
